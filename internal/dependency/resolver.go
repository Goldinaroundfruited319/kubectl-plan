...
func (r *Resolver) Resolve(targetKind, targetName, targetNamespace string) (*DependencyGraph, error) {
			...
			for _, rs := range targetRS {
				for _, ref := range pod.OwnerReferences {
					if ref.Kind == "ReplicaSet" && ref.Name == rs.Name {
						...
						graph.AddEdge(Edge{
							From:         NodeKey(targetNode.Namespace, targetNode.Kind, targetNode.Name),
							To:           NodeKey(podNode.Namespace, podNode.Kind, podNode.Name),
							Relationship: RelOwns,
							Depth:        1,
							Confidence:   1.0,
							Evidence: []Evidence{
								{
									Type:        EvidenceOwnerRef,
									Source:      SourceK8sAPI,
									Description: fmt.Sprintf("ownerReference via ReplicaSet %s", rs.Name),
									Confidence:  1.0,
								},
							},
						})
						...
						Evidence: []Evidence{
							{
								Type:        EvidenceOwnerRef,
								Source:      SourceK8sAPI,
								Description: "direct ownerReference",
								Confidence:  1.0,
							},
						},
					})
	...
	for _, svc := range r.data.Services {
		...
		if matches {
			...
			graph.AddEdge(Edge{
				...
				Relationship: RelSelects,
				Depth:        1,
				Confidence:   0.95,
				Evidence: []Evidence{
					{
						Type:        EvidenceLabelSelector,
						Source:      SourceK8sAPI,
						Description: fmt.Sprintf("selector %v matches target pods/labels", svc.Spec.Selector),
						Confidence:  0.95,
					},
				},
			})
				...
				for _, svc := range matchedServices {
					if path.Backend.Service.Name == svc.Name {
						...
						graph.AddEdge(Edge{
							...
							Relationship: RelRoutes,
							Depth:        2,
							Confidence:   0.95,
							Evidence: []Evidence{
								{
									Type:        EvidenceIngressBackend,
									Source:      SourceK8sAPI,
									Description: fmt.Sprintf("rules route %s -> service %s", path.Path, svc.Name),
									Confidence:  0.95,
								},
							},
						})
	...
	for _, d := range r.data.Deployments {
		...
		r.scanWorkloadEnv(graph, targetNode, matchedServices, d.Name, d.Namespace, "Deployment", d.Spec.Template.Spec.Containers, d.Labels)
	}
	for _, s := range r.data.StatefulSets {
		if s.Name == targetName && s.Namespace == targetNamespace {
			continue
		}
		r.scanWorkloadEnv(graph, targetNode, matchedServices, s.Name, s.Namespace, "StatefulSet", s.Spec.Template.Spec.Containers, s.Labels)
	}
	for _, d := range r.data.DaemonSets {
		if d.Name == targetName && d.Namespace == targetNamespace {
			continue
		}
		r.scanWorkloadEnv(graph, targetNode, matchedServices, d.Name, d.Namespace, "DaemonSet", d.Spec.Template.Spec.Containers, d.Labels)
	}
				...
				if from.PodSelector != nil {
					...
					if matches {
						...
						graph.AddEdge(Edge{
							...
							To:           NodeKey(targetNode.Namespace, targetNode.Kind, targetNode.Name),
							Relationship: RelNetworkPolicy,
							Depth:        1,
							Confidence:   0.80,
							Evidence: []Evidence{
								{
									Type:        EvidenceNetworkPolicy,
									Source:      SourceK8sAPI,
									Description: fmt.Sprintf("ingress policy %s restricts access to target", policy.Name),
									Confidence:  0.80,
								},
							},
						})
			...
			for key, val := range cm.Data {
				if strings.Contains(val, svc.Name) {
					...
					graph.AddEdge(Edge{
						...
						To:           NodeKey(svc.Namespace, svc.Kind, svc.Name),
						Relationship: RelVolumeRef,
						Depth:        2,
						Confidence:   0.60,
						Evidence: []Evidence{
							{
								Type:        EvidenceVolumeMount,
								Source:      SourceK8sAPI,
								Description: fmt.Sprintf("ConfigMap key %s references service %s", key, svc.Name),
								Confidence:  0.60,
								RawValue:    val,
							},
						},
				...
				Evidence: []Evidence{
					{
						Type:        EvidenceLabelSelector,
						Source:      SourceK8sAPI,
						Description: fmt.Sprintf("PDB matches target pod labels (minAvailable=%v)", pdb.Spec.MinAvailable),
						Confidence:  0.95,
					},
				},
			})
				...
				Evidence: []Evidence{
					{
						Type:        EvidenceLabelSelector,
						Source:      SourceK8sAPI,
						Description: fmt.Sprintf("HPA targets %s/%s directly", targetNode.Kind, targetNode.Name),
						Confidence:  0.95,
					},
				},
			})
